...
use crate::{ constants::{ GLOBAL_STATE_SEED, USER_INFO_SEED, VAULT_SEED }, errors::*, events::{DepositEvent, WithdrawEvent}, state::{GlobalState, UserInfo} };
...
pub fn deposit(ctx: Context<Deposit>, amount: u64) -> Result<()> {
    ...
    let cpi_ctx: CpiContext<'_, '_, '_, '_, Transfer<'_>> = CpiContext::new(accts.token_program.to_account_info(), Transfer {
...
pub fn withdraw(ctx: Context<Withdraw>, amount: u64) -> Result<()> {
    ...
    let cpi_ctx = CpiContext::new(accts.token_program.to_account_info(), Transfer {
    ...
    transfer(
        cpi_ctx.with_signer(signer), amount
    )?;
