...
describe("spl_payment", () => {
  ...
  const tokenMint = new PublicKey(
    "8NtheYSKWDkCgWoc8HScQFkcCTF1FiFEbbriosZLNmtE"
  );
  ...
  it("is initialized accounts", async () => {
    ...
    [tokenVaultAccount, tokenVaultAccountBump] =
      await anchor.web3.PublicKey.findProgramAddress(
    ...
    [userInfo, userInfoBump] = await anchor.web3.PublicKey.findProgramAddress(
      [Buffer.from(USER_INFO_SEED), user.publicKey.toBuffer()],
      program.programId
    );
  ...
  it("update owner", async () => {
    const new_owner = new PublicKey(
      "7bVNisSuPayDrdBdNh7uiJNSiLrjatnADPSTLWBoUUHb"
    );

    const tx = await program.rpc.updateOwner(new_owner, {
      accounts: {
        owner: owner.publicKey,
        globalState,
        systemProgram: SystemProgram.programId,
  ...
  it("deposit", async () => {
    ...
    try {
      ...
      const event = await new Promise<Event[E]>(async (res) => {
        ...
        const tx = await program.rpc.deposit(new anchor.BN(depositAmount), {
          accounts: {
            user: user.publicKey,
            globalState,
            userInfo,
  ...
  it("withdraw", async () => {
    ...
    try {
      ...
      const event = await new Promise<Event[E]>(async (res) => {
        ...
        const tx = await program.rpc.withdraw(new anchor.BN(withdrawAmount), {
          accounts: {
            user: user.publicKey,
            globalState,
            userInfo,
